    ...
    def run(self, prompt: str) -> SamplingResult:
        ...
        for attempt in range(attempts):
            try:
                ...
                responses = [str(sample.get("text", "")) for sample in generations]
                tokens = [int(sample.get("num_generated_tokens", 0)) for sample in generations]
                avg_tokens = float(sum(tokens) / max(len(tokens), 1))
                canonical_answers = [self._canonicalize(r) for r in responses]
                vote_counter = Counter(canonical_answers)
                if not vote_counter:
                    raise BackendError("No responses generated by backend")
                # Choose the answer with the highest vote count; tie-break by earliest occurrence.
                canonical_choice, _ = max(
                    vote_counter.items(),
                    key=lambda item: (item[1], -canonical_answers.index(item[0])),
                )
                # Map canonical answer back to the first raw response that produced it.
                chosen_raw = next(
                    resp
