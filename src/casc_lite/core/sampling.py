...
class SamplingResult:
    ...
    vote_margin: float
    second_vote_ratio: float
    token_length_std: float
    avg_logprob: float | None
    logprob_std: float | None
    ...
    def run(self, prompt: str) -> SamplingResult:
        ...
        for attempt in range(attempts):
            try:
                ...
                canonical_choice, _ = max(
                    vote_counter.items(),
                    key=lambda item: (item[1], -canonical_answers.index(item[0])),
                )
                avg_tokens = float(sum(tokens) / max(len(tokens), 1))
                token_length_std = float(np.std(tokens, ddof=0)) if len(tokens) > 1 else 0.0
                total_votes = len(responses)
                if vote_counter and total_votes > 0:
                    sorted_counts = sorted(vote_counter.values(), reverse=True)
                    top_votes = sorted_counts[0]
                    second_votes = sorted_counts[1] if len(sorted_counts) > 1 else 0
                    vote_margin = top_votes / total_votes
                    second_vote_ratio = second_votes / total_votes
                else:
                    vote_margin = 0.0
                    second_vote_ratio = 0.0
                if sample_logprobs:
                    avg_logprob = float(np.mean(sample_logprobs))
                    logprob_std = float(np.std(sample_logprobs, ddof=0)) if len(sample_logprobs) > 1 else 0.0
                else:
                    avg_logprob = None
                    logprob_std = None
                chosen_raw = next(
